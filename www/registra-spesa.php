...
if(isset($_POST['inseriscispesa'])){
	...
	$inserisci_spesa = "INSERT INTO spesa (negozio, id_ambiente, id_utente, data) VALUES ('{$_POST['negozio']}', {$_GET['ambiente']}, '{$_SESSION['email']}', '{$_POST['data']}');";
	...
	foreach($_POST['t'] as $prodotto){
		...
		query("UPDATE appartenenza SET saldo=saldo+{$prodotto['costo']} WHERE id_utente='{$_SESSION['email']}' AND id_ambiente={$_GET['ambiente']}");
		
		$costo_per_utilizzatore = $prodotto['costo'] / count($prodotto['id_utente']);
		foreach($prodotto['id_utente'] as $utente){
...
<table>
	...
	<tr>
		...
		<?php
		foreach($utenti as $utente){
			echo "<td><input type='checkbox' name='t[1][id_utente][]' value='{$utente['id_utente']}' checked/></td>";
		}
		?>
